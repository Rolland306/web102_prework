...
function addGamesToPage(games) {
    ...
    for (let index = 0; index < games.length; index++) {
        ...
        div.innerHTML = `<img class="game-img" src=${element["img"]} /> <p><h3>${element["name"]}</h3></p> <p>${element["description"]}</p> <p>Backers:${element["backers"]}</p>`;
...
const num_contributions = GAMES_JSON.reduce( (acc, currValue) => {
    return acc + currValue["backers"];
},0);


// set the inner HTML using a template literal and toLocaleString to get a number with commas
contributionsCard.innerHTML = `${num_contributions.toLocaleString('en-US')}`;


// grab the amount raised card, then use reduce() to find the total amount raised
const raisedCard = document.getElementById("total-raised");
const total_raised = GAMES_JSON.reduce( (acc, currValue) => {
    return acc + currValue["pledged"];
},0);
...
const displayStr = `A total of $${total_raised.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} gam${num_unfunded>1 ? "es" : "e"}. 
Currently, ${num_unfunded} gam${num_unfunded>1 ? "es" : "e"} remai${num_unfunded>1 ? "n" : "ns"} unfunded. We need 
